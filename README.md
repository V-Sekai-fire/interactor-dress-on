...
| [8](gates/8-loop/) | The loop: body → rig → pen → curvenet → mesh → fit → check → drape, `project/stages/` composed by `pipeline.gd` | **FAIL at MESH, not yet end to end** — on main's ELFs with the FoxGirl body and rig fixtures the scripted pen reaches curvenet (2 cycles, 2 patches) but the patches are the ring caps, so the mesh is 2 components; past that, fit.elf fits the LCL skirt in the loop (2446 s / 2714 s, `OK none`, control INTERSECTS) and the drape runs 100 finite rd steps at 38.8 ms/step. Every one of the 113 guest entry points has a `/root/Main` wrapper (audited) |
