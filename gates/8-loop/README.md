...
| **Gate 8 flat + psd** (2026-09-23; `fit_force_psd`, polysolve's `force_psd_projection` in both of the fit's solves, default on since cut-6e) | **PASS FIXTURE:infer,rig**: FIT_RUN **627 s** (1393 s without), 150 Newton (253), final energy 0.0112336 (0.0112073, +0.23%), CHECK `OK none`, the push-vertex control INTERSECTS; the whole loop 643 s. Units 43/43. Why psd (gates/6-fit/amdahl): plain Newton on this nonconvex energy takes CCD-clipped steps and the AL's minimizes run to their cap; the projected Hessian's directions let the AL converge in one minimize | `flat-psd.*`, `units.txt` |
