...
--siblings FILE (kernels/ggml/cpp_siblings.txt) also checks every
"<kernel> <sibling>" pair: both are kernels of this table, the sibling is
not itself GPU-only, and the two have the same thread-group size (the host
harness runs the sibling over the kernel's grid).
"""
...
def read_siblings(path):
    ...
    for n, line in enumerate(open(path), 1):
        ...
        if not words:
            continue
        if len(words) != 2:
            raise ValueError("%s:%d: want '<kernel> <sibling>', got %r" % (path, n, line.strip()))
        pairs.append((words[0], words[1]))
    return pairs


def check_siblings(pairs, sizes):
    errors = []
    gpu_only = set(k for k, _ in pairs)
    for k, s in pairs:
        if k not in sizes or s not in sizes:
            errors.append("sibling pair %s -> %s: both must be kernels of this table" % (k, s))
        elif s in gpu_only:
            errors.append("sibling pair %s -> %s: the sibling is GPU-only itself" % (k, s))
        elif sizes[k] != sizes[s]:
            errors.append("sibling pair %s -> %s: thread groups %s and %s differ" % (k, s, sizes[k], sizes[s]))
    return errors
...
def main():
    ...
    for k in a.kernels:
        try:
            ...
            errors.append(str(e))
    if a.siblings and not errors:
        try:
            errors += check_siblings(read_siblings(a.siblings), dict(parsed))
        except ValueError as e:
            errors.append(str(e))
    if errors:
        for e in errors:
