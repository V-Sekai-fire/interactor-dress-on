...
#   gates/0f-runtime/ggml_host/build.sh   -> ../ggml_host.txt, read by gate_runtime.gd
set -euo pipefail
...
B="${HOST_BUILD_DIR:-C:/b/ggml-host-0f}"
if [ ! -f "$B/build.ninja" ]; then
	cmake -S "$(cygpath -m "$HERE")" -B "$B" -G Ninja -DCMAKE_MAKE_PROGRAM="$NINJA" \
		...
		-DCMAKE_BUILD_TYPE=Release -DGGML_SRC="${GGML_SRC:-C:/contract-manifest/2-contract/ggml}"
fi
