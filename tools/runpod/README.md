...
from this GitHub repo; three endpoints (composition, AGENTS.md rule 6).
...
| `voxhammer` | `tools/runpod/voxhammer/Dockerfile` | image-conditioned 3D edit, OpenUSD in and out | 48 GB (fills a 24 GB card) | network volume, `/runpod-volume/models` |

All three are **queue** endpoints, not load-balancing ones: a load-balanced request
is capped at 5.5 min, a Pixal3D or VoxHammer job takes 4–5 min plus a cold start,
and one loop job takes ~25 min (the fit). Jobs go to
...
2. **Serverless → New Endpoint → Import Git Repository**, three times, choosing
...
| | dress-on-loop | pixal3d | voxhammer |
|---|---|---|---|
| workers min/max | 0 / 1 | 0 / 1 | 0 / 1 |
| idle timeout | 5 s | 60 s | 60 s |
| execution timeout | 3600 s | 900 s | 900 s |
| FlashBoot | on | on | on |
| container disk | 20 GB | 40 GB | 40 GB |
| env | `NVIDIA_DRIVER_CAPABILITIES=all` (in the image) | `MODELS_DIR=/runpod-volume/models` | `MODELS_DIR=/runpod-volume/models` |
...
`pixal3d` / `voxhammer`: `{"route": "predict" | "extract" | "health", ...}` with
